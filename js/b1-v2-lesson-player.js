...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const lesson = Array.isArray(window.B1_V2_LESSONS) ? window.B1_V2_LESSONS.find((item) => item.number === lessonNumber) : null;
    ...
    if (!lesson) {
        document.getElementById('slides-root').innerHTML = `
            <div class="surface rounded-[2rem] p-10 text-center">
                ...
                <p class="text-slate-600">We could not load this B1-V2 lesson data.</p>
            </div>
        `;
        return;
    }
    ...
    document.getElementById('lesson-title').textContent = `B1-V2 - Lesson ${lesson.number}: ${lesson.title}`;
    document.title = `B1-V2 - Lesson ${lesson.number}: ${lesson.title} | Ingles no seu Ritmo`;
...
function getRestructurePack(lesson) {
    ...
    return {
        title: plan.title || 'B1-V2 Premium Upgrade',
...
function handleQuizClick(event, runtime) {
    ...
    options.forEach((option) => {
        ...
        setQuizOptionBadge(option, isCorrect ? 'âœ“' : '');
    });
