document.addEventListener('DOMContentLoaded', () => {
    ...
    const lesson = Array.isArray(window.B1_V2_LESSONS)
        ? window.B1_V2_LESSONS.find(item => item.number === lessonNumber)
        : null;
    ...
    document.getElementById('lesson-title').textContent = `B1-V2 - Lição ${lesson.number}: ${lesson.title}`;
    document.title = `B1-V2 - Lição ${lesson.number}: ${lesson.title} | Inglês no seu Ritmo`;
        ...
        <div class="lesson-stage">
            ...
            <div class="b1-vocab-grid mt-7">
                ${items.map(item => {
                    ...
                    return `
                        <button type="button" class="b1-vocab-card" aria-pressed="false">
...
function renderMusicSlide(slide, lesson, slideIndex) {
    ...
    const discussion = Array.isArray(slide.discussion) ? slide.discussion : [];
        ...
        <div class="lesson-stage">
            ...
            <div class="b1-music-layout mt-7">
                ...
                <div class="b1-lyric-panel">
                    <p class="prep-section-label">Lyric practice</p>
                    <p class="b1-lyric-text mt-4">
                        ${lines.map((line, index) => {
                            const [before, answer, after] = Array.isArray(line) ? line : [line.before, line.answer, line.after];
                            const answerId = `music-answer-${lesson.number}-${slideIndex}-${index}`;
                            return `
                                ${escapeHtml(before)}
                                <span class="b1-music-gap">
                                    <span class="music-placeholder">________</span>
                                    <strong id="${answerId}" class="music-answer hidden">${escapeHtml(answer)}</strong>
                                </span>
                                ${renderRevealButton(answerId, 'Revelar', 'Ocultar', 'fa-eye')}
                                ${escapeHtml(after)}${index < lines.length - 1 ? '<br>' : ''}
                            `;
                        }).join(' ')}
                    </p>
                </div>
            ...
            ${discussion.length ? `
                <div class="b1-music-questions mt-7">
                    ${discussion.map((question, index) => `<p><span>${index + 1}</span>${escapeHtml(question)}</p>`).join('')}
                </div>
            ` : ''}
        </div>
    `);
