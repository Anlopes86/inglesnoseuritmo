...
if (!/station\.items\.slice\(0, 3\)/.test(a1Renderer)) fail('A1-V3 reviews are not capped for a 60-minute lesson.');
if (!/CEFR Progress Check/.test(a1Renderer) || !/renderCanDoAssessment/.test(a1Renderer)) fail('A1-V3 reviews lack a CEFR-oriented Can-Do assessment.');
if (/slide\('Music Review'/.test(a1Renderer)) fail('A1-V3 reviews still use a music stage instead of performance assessment.');
if (!/Letra com lacunas — modelo editorial/.test(a1Renderer) || !/Array\.from\(\{ length: 6 \}/.test(a1Renderer)) fail('A1-V3 music placeholder is not ready for six lyric-gap lines.');
...
if (!/Letra com lacunas — modelo editorial/.test(a2Renderer) || !/Array\.from\(\{ length: 6 \}/.test(a2Renderer)) fail('A2-V3 music placeholder is not ready for six lyric-gap lines.');

if (!/Station 9: Can-Do Assessment/.test(a2Renderer) || !/assessmentCriteria/.test(a2Renderer)) fail('A2-V3 reviews lack a CEFR-oriented Can-Do assessment.');
...
if (!/Letra com lacunas — modelo editorial/.test(b1Renderer) || !/Array\.from\(\{ length: 6 \}/.test(b1Renderer)) fail('B1-V3 music placeholder is not ready for six lyric-gap lines.');
...
for (const lesson of b1Lessons) {
    ...
    if (reviewSet.has(lesson.number)) {
        ...
        if (!types.includes('assessment')) fail(`B1-V3 L${lesson.number}: review lacks assessment.`);
    } else if (!types.includes('speaking')) {
        fail(`B1-V3 L${lesson.number}: regular lesson lacks a live speaking task.`);
    }
}

const home = read('home-aluno.html');
for (const module of modules) {
