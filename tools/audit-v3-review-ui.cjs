...
async function main() {
    ...
    try {
        ...
        await render({ name: 'b1-review-game-dark', file: 'b1-v3/licao-16.html', selector: '.slide:has([data-v3-memory-board])', width: 1440, height: 1000, expect: 'memory', action: `const cards = target.querySelectorAll('[data-v3-memory-card]'); const first = cards[0]; const pair = [...cards].find(card => card !== first && card.dataset.pairId === first.dataset.pairId); first.click(); pair.click();` });
        ...
        await render({ name: 'b1-review-grammar-dark', file: 'b1-v3/licao-16.html', selector: '.slide[data-slide-type="grammar"]', width: 1440, height: 1000, expect: 'layout' });
        await render({ name: 'a1-draft-safe-homework', file: 'a1-v3/licao-01.html', selector: '.slide[data-title="Homework"]', width: 1440, height: 1000, expect: 'layout' });
        await render({ name: 'a2-draft-safe-homework-mobile', file: 'a2-v3/licao-01.html', selector: '.slide[data-title="Homework"]', width: 390, height: 844, expect: 'layout' });
        await render({ name: 'b1-draft-safe-homework-dark', file: 'b1-v3/licao-01.html', selector: '.slide[data-slide-type="homework"]', width: 1440, height: 1000, expect: 'layout' });
    } finally {
