...
for(let n=1;n<=2;n++){errors=[];await page.goto(pathToFileURL(path.join(root,'b1-v3/licao-'+String(n).padStart(2,'0')+'.html')).href);
...
await page.locator('#theme-toggle').click();const theme=await page.locator('html').getAttribute('data-theme');await page.reload();assert.equal(await page.locator('html').getAttribute('data-theme'),theme);await page.screenshot({path:path.join(out,'dark-homework.png')});fs.writeFileSync(path.join(out,'results.json'),JSON.stringify(results,null,2));console.log('PASS: 2 lessons, all sections, answers, music ordering, infinitives, offline completion, theme persistence and responsive width.');
