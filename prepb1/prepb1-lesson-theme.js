document.addEventListener('DOMContentLoaded', () => {
    ...
    function syncThemeToggle() {
        const button = document.querySelector('[data-prepb1-theme-toggle]');
        if (!button) return;
    ...
    function injectThemeToggle() {
        ...
        if (!headerContainer || !slideCounter || headerContainer.querySelector('[data-prepb1-theme-toggle]')) {
            return;
        }
        ...
        themeButton.className = 'prep-theme-toggle';
        themeButton.setAttribute('data-prepb1-theme-toggle', 'true');
    ...
    const backLink = document.querySelector('header a[href="prepb1.html"]');
    const prevBtn = document.getElementById('prev-btn');
    const nextBtn = document.getElementById('next-btn');
    const finishBtn = document.getElementById('finish-btn') || document.getElementById('finish-lesson-btn');

    if (backLink) {
        backLink.innerHTML = '<i class="fas fa-chevron-left"></i> Voltar para a ponte A2-B1';
    }
    ...
    document.addEventListener('click', (event) => {
        const themeToggle = event.target.closest('[data-prepb1-theme-toggle]');
    ...
    document.body.classList.add('prepb1-theme-ready');
});
