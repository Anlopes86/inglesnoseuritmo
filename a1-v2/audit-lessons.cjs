...
const dataPath = path.join(__dirname, 'a1-v2-data.js');
const context = { window: {} };
vm.runInNewContext(fs.readFileSync(dataPath, 'utf8'), context, { filename: dataPath });

const data = context.window.A1V2_DATA;
const errors = [];
...
function validateReview(review, lessonNumber) {
    ...
    validateMusic(review.music, lessonNumber);
    validateHomework(review.homework, lessonNumber);
}

if (!data || !Array.isArray(data.lessonTitles) || data.lessonTitles.length !== 32) {
    fail('A1 V2 must expose exactly 32 lesson titles.');
}
if (!Array.isArray(data?.unitLabels) || data.unitLabels.length !== 32) {
    fail('A1 V2 must expose exactly 32 unit labels.');
}

const moduleSource = fs.readFileSync(path.join(__dirname, '..', 'js', 'a1-v2-module.js'), 'utf8');
for (const [variableName, expected] of [['lessonTitles', data?.lessonTitles], ['unitLabels', data?.unitLabels]]) {
    ...
    if (!match) {
        fail(`A1 V2 module is missing ${variableName}.`);
        continue;
    }
    try {
        const actual = JSON.parse(`[${match[1]}]`);
        if (JSON.stringify(actual) !== JSON.stringify(expected)) {
            fail(`A1 V2 module ${variableName} does not match the lesson data.`);
        }
    } catch {
        fail(`A1 V2 module ${variableName} could not be parsed.`);
    }
}
...
for (let lessonNumber = 1; lessonNumber <= 32; lessonNumber += 1) {
    ...
    if ((html.match(/a1-v2-data\.js/g) || []).length !== 1) fail(`Lesson ${lessonNumber}: data script must load once.`);
    if ((html.match(/a1-v2-lesson-content\.js/g) || []).length !== 1) fail(`Lesson ${lessonNumber}: renderer must load once.`);
...
const rendererSource = fs.readFileSync(path.join(__dirname, 'a1-v2-lesson-content.js'), 'utf8');
if (/\bdiego\b/i.test(allSource)) fail('Forbidden name Diego found in A1 V2 content.');
if (/record (yourself|your voice)|gravar? (sua )?voz|grave (um )?[áa]udio|use (o )?microfone|envie (um )?[áa]udio/i.test(allSource)) {
    fail('A recording or microphone activity was found in A1 V2 content.');
}
if (/Let.?s practice this situation|Good\. Now continue|One more short exchange/i.test(allSource)) {
    fail('Generic dialogue template language found in A1 V2 content.');
}
if (/open\.spotify\.com\/embed\/search/i.test(rendererSource)) {
    fail('A1 V2 renderer still uses a generic Spotify search instead of direct tracks.');
}

if (errors.length) {
    console.error(`A1 V2 audit failed with ${errors.length} issue(s):`);
    for (const error of errors) console.error(`- ${error}`);
    process.exitCode = 1;
} else {
    console.log('A1 V2 audit passed: 24 regular lessons, 8 review circuits, and 32 generated pages checked.');
}
