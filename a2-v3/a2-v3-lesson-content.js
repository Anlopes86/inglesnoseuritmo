(function () {
    ...
    function getLessonData() {
        ...
        const bank = Object.assign({}, baseBank, profile);

        bank.label = profile.label || baseBank.label || title.toLowerCase();
        bank.matchLabel = `${title.toLowerCase()} ${baseBank.label || ''}`;
        bank.introDialogue = profile.introDialogue || lessonSpecific.intro || null;
        bank.practice = profile.practice || createPracticeItems(bank, number);
        bank.translations = profile.translations || createTranslations(bank, title, number);
        bank.expressionTranslations = profile.expressionTranslations || createExpressionTranslations(bank, number);
        bank.dialogues = profile.dialogues || lessonSpecific.dialogues || createDialogues(bank, title);
        const followUp = dialogueFollowUps[number];
        if (followUp && bank.dialogues.length) {
        ...
        const readingUpgrade = readingUpgrades[number];
        if (readingUpgrade) {
    ...
    function fillMusic(bank) {
        ...
        const lyricMarkup = `<div class="v3-lyric-placeholder">
    ...
    function reviewTableRows(review) {
    ...
    function fillReviewLesson(data, review) {
        ...
        document.querySelector('.slide[data-title="Vocabulary Flashcards"] h2').textContent = 'Station 1: Grammar Memory Cards';
        setHtml('#flashcards-container', review.focus.map((item, index) => {
            const expanded = expandReviewFocus(item);
            return `
            <div class="activity-card p-5 min-h-52">
                <p class="generated-tag mb-3">Card ${index + 1}</p>
                <h3 class="text-xl font-black text-emerald-700 mb-2">${escapeHtml(expanded.name)}</h3>
                <p class="text-slate-700 font-semibold">${escapeHtml(expanded.detail)}</p>
                <p class="text-sm text-slate-500 mt-4">${escapeHtml(expanded.extra)}</p>
            </div>
        `; }).join(''));
        ...
        setHtml('#oral-translation-1', review.translations.map(([pt, en], index) => `
            <div class="activity-card p-5">
                <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        setHtml('#oral-translation-2', `
            ...
            ${renderReviewItems(review.oralTest || [])}
        `);
        document.querySelector('.slide[data-title="Music Moment"] h2').textContent = 'Station 9: Can-Do Assessment';
        const musicPrompt = document.querySelector('.slide[data-title="Music Moment"] .section-card > p');
        if (musicPrompt) musicPrompt.textContent = 'Avalie evidências produzidas ao longo do circuito. Não apresente linguagem nova nesta etapa.';
        const assessmentCriteria = [
            ['Integração do conteúdo', 'Combina as funções e estruturas revisadas para cumprir a tarefa.'],
            ['Desenvolvimento e conexão', 'Produz respostas conectadas, além de frases isoladas.'],
            ['Interação', 'Responde, pede esclarecimento e faz ao menos uma pergunta de continuidade.'],
            ['Controle e inteligibilidade', 'Mantém a mensagem clara mesmo quando ainda há erros.']
        ];
        setHtml('#music-lyrics', `
            <div class="v3-can-do-assessment text-left">
                <p class="v3-assessment-scale">Escala: 1 = ainda não · 2 = com bastante apoio · 3 = com pouco apoio · 4 = com autonomia</p>
                ${assessmentCriteria.map(([title, descriptor]) => `
                    <label class="v3-assessment-row">
                        <span><strong>${escapeHtml(title)}</strong><span>${escapeHtml(descriptor)}</span></span>
                        <select aria-label="Avaliação: ${escapeHtml(title)}"><option value="">Selecionar</option><option>1 — Ainda não</option><option>2 — Com apoio</option><option>3 — Quase autônomo</option><option>4 — Autônomo</option></select>
                    </label>
                `).join('')}
                <label class="v3-assessment-notes">Evidência observada e próximo passo<textarea rows="3" placeholder="Registre uma evidência concreta e uma prioridade para a próxima aula."></textarea></label>
            </div>
        `);
        setHtml('#homework-list', `
            <li class="block">
