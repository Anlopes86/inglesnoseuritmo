...
const contentPath = path.join(__dirname, 'a2-v3-lesson-content.js');
...
const window = { location: { pathname: '/a2-v3/licao-01.html' } };
...
for (let number = 1; number <= 32; number += 1) {
    ...
    const contentScriptCount = (lessonHtml.match(/a2-v3-lesson-content\.js/g) || []).length;
    ...
    window.location.pathname = `/a2-v3/licao-${String(number).padStart(2, '0')}.html`;
    document.title = `A2 V3 Lesson ${number}`;
...
if (/open\.spotify\.com\/embed\/search/i.test(contentSource)) {
    fail('A2 V3 renderer still uses a generic Spotify search instead of direct tracks.');
}
...
if (!/window\.location\.href\s*=\s*['"]a2-v3\.html['"]/i.test(contentSource)) {
    fail('Finish button does not have the a2-v3.html fallback.');
}

if (!/['"]a2-v3['"]\s*:\s*['"]a2-v3\.html['"]/.test(progressManagerSource)) {
    fail('Progress manager does not map a2-v3 progress to the a2-v3.html landing page.');
}

if (errors.length) {
    console.error(`A2 V3 audit failed with ${errors.length} issue(s):`);
    for (const error of errors) console.error(`- ${error}`);
    process.exitCode = 1;
} else {
    console.log('A2 V3 audit passed: 32 lessons checked, including 4 review circuits.');
}
