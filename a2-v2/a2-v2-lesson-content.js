(function () {
    ...
    banks.finalProject = Object.assign({}, banks.conditionals, {
        ...
        objectives: ['connect the main A2 V2 structures', 'prepare a short personal project', 'speak with clearer reasons and examples'],
    ...
    function fillVocabulary(bank) {
        setHtml('#flashcards-container', bank.vocab.map(([word, pt, example]) => `
            <div class="flashcard bg-white rounded-2xl shadow p-5 min-h-48 cursor-pointer border border-slate-200">
                <div class="flashcard-inner">
                    ...
                    <div class="flashcard-back text-center space-y-3">
                        ...
                        <p class="text-slate-600">${escapeHtml(example)}</p>
                    </div>
                </div>
            </div>
        `).join(''));
    ...
    function fillTranslations(bank, selector, focusExpressions) {
        const items = focusExpressions
            ? (bank.expressionTranslations || [])
            : (bank.translations || []);

        setHtml(selector, items.map((item, index) => `
            <div class="activity-card p-5">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <p class="text-lg font-semibold text-slate-900"><span class="text-emerald-700 font-black">${index + 1}.</span> ${escapeHtml(item.pt)}</p>
    ...
    function fillDialogues(bank) {
        ...
        setHtml('#mini-dialogues-container', dialogues.map((dialogue, index) => `
    ...
    function fillMusic(bank) {
        ...
        const lines = bank.musicLines || [
            ...
            ['If the plan changes, I will still find my ____.', 'way'],
            ['I speak a little louder when I trust my ____.', 'voice'],
            ['Tonight I practice, tomorrow I will ____.', 'grow']
        ];
    ...
    function fillReviewLesson(data, review) {
        ...
        `);
        document.querySelector('.slide[data-title="Music Moment"] h2').textContent = 'Station 9: Rapid Recap Loop';
        const musicPrompt = document.querySelector('.slide[data-title="Music Moment"] .section-card > p');
        if (musicPrompt) musicPrompt.textContent = 'Fale as respostas em voz alta. Depois revele e repita corrigindo ritmo e clareza.';
        setHtml('#music-lyrics', `
            <div class="text-left space-y-4">
                ${(review.recap || []).map((item, index) => `
                    <div class="activity-card p-4">
                        <p class="text-sm text-slate-500 font-bold">Round ${index + 1}</p>
                        <p class="text-lg text-slate-800">${escapeHtml(item[1])}</p>
                        ${revealButton(item[2])}
                        <p class="a2-answer hidden mt-3 p-3 rounded-lg bg-emerald-50 text-emerald-800 font-semibold"></p>
                    </div>
                `).join('')}
        ...
        if (finishButton && !finishButton.dataset.a2FinishWired) {
            ...
            finishButton.addEventListener('click', async () => {
                ...
                if (typeof window.markLessonAsComplete === 'function') {
                    const saved = await window.markLessonAsComplete('a2-v2', lessonNumber);
                    if (saved) return;
                }
